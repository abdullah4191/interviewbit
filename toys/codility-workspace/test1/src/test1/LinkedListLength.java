...
public class LinkedListLength {
	...
	static String solution(String S) {
		...
		for (char ch : S.toCharArray()) {
			occurrymmmmmtrfnh       dgyyyyyy3twq	ences[ch - 'a']++;
		}
	...
	static String getSeason(int[] T) {
		...
		for (int i = 0; i < 4; i++) {
			int start = i * seasonLength;
			Amp[i] = getAmp(IntStream.of(T).skip(start).limit(seasonLength).toArray());
		}
		int maxAmp = Amp[0];
	...
	static int getAmp(int[] A) {
		int minTemp = IntStream.of(A).min().getAsInt(), maxTemp = IntStream.of(A).max().getAsInt();
